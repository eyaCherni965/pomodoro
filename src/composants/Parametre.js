...
const [pomodoro, setPomodoro] = useState(() => parseInt(localStorage.getItem("pomodoro")) || 25);
const [pause, setPause] = useState(() => parseInt(localStorage.getItem("pause")) || 5);
...
const handlePauseChange = (e) => {
  ...
  return (
    <div className="w-screen h-screen relative bg-gradient-to-r from-blue-100 to-violet-300 flex flex-col items-center justify-center">
      {/* Titre principal */}
      <p className="text-violet-700 font-semibold absolute top-10 left-10 text-1xl sm:text-2xl">
      ...
      <div className="w-[90%] max-w-md border-8 border-violet-400 bg-violet-200 rounded-xl shadow-xl p-6 flex flex-col gap-6">
        <div className="flex flex-col">
          <label className="font-semibold text-violet-700 mb-1">Durée du Pomodoro (en minutes)</label>
          <input
            type="number"
            min="0"
        ...
        <div className="flex flex-col">
          <label className="font-semibold text-violet-700 mb-1">Durée de la pause (en minutes)</label>
          <input
            type="number"
            min="0"
